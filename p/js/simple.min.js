...
$scope.settings=settings;$scope.now={a:-1,b:-1},$scope.element={a:null,b:null},$scope.clickA=function(a,index){-1==$scope.now.a?($scope.element.a=a,$scope.now.a=index,$scope.now.b=-1,ga("send","event","Cell_A","Opened",index+"_"+a.title)):(ga("send","event","Cell_A","Closed",index+"_"+a.title),$scope.element.a=null,$scope.now.a=-1,$scope.now.b=-1),$scope.log($scope.now)},$scope.clickB=function(a,aindex,b,bindex){if($scope.now.b!=bindex){$scope.element.b=b,$scope.now.a=aindex,$scope.now.b=bindex;var st="";st+="img"==b.type?b.img:"",st+="iframe"==b.type?b.iframe:"",st+="vimeo"==b.type?b.vimeo:"",ga("send","event","Cell_B","Opened",aindex+"_"+bindex+"_"+b.type+"_"+st)}else{var st="";st+="img"==b.type?b.img:"",st+="iframe"==b.type?b.iframe:"",st+="vimeo"==b.type?b.vimeo:"",ga("send","event","Cell_B","Closed",aindex+"_"+bindex+"_"+b.type+"_"+st),$scope.element.b=null,$scope.now.b=-1}$scope.log($scope.now)},$document.bind("keydown keypress",function(event){27==event.which&&(-1==$scope.now.b?($scope.element.a=null,$scope.now.a=-1):($scope.element.b=null,$scope.now.b=-1),$scope.$apply()),37==event.which,39==event.which}),$http.get("../media.yml").success(function(res){$scope.root=jsyaml.load(res)}).error(function(data,status,headers,config){}),$scope.log=function(e){}}]),angular.module("jdlf.directives",[]).directive("backImg",function(){return function(scope,element,attrs){var url=attrs.backImg;element.css({"background-image":"url("+url+")"})}}).directive("href",function(){return{compile:function(element){element.attr("target","_blank")}}}),angular.module("jdlf.filters",[]).filter("arrimgfy",function(){return function(str){return str.trim().split(/\n/g)}}).filter("allowvimeo",function($sce){return function(url){var vid=url.match(/[0-9]+/)[0],options="?title=0&portrait=0&badge=0&byline=0&color=ffffff";return $sce.trustAsResourceUrl("//player.vimeo.com/video/"+vid+options)}}).filter("allowurl",function($sce){return function(url){return $sce.trustAsResourceUrl(url)}}).filter("adjoin",function(){return function(list,max){return list.join(", ")}});